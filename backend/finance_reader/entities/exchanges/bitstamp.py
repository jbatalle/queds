...
class Bitstamp(AbstractExchange):
    ...
    def get_orders(self):
        self._logger.info("Get Bitstasmp Closed orders")
    ...
    def get_transactions(self):
        ...
        self._logger.info("Converting {0} orders".format(len(orders)))
        transactions = self._convert_deposit_orders(orders)
        transactions.extend(self._convert_withdrawal_orders(orders))
    ...
    def _convert_withdrawal_orders(self, orders):
        transactions = []
        for o in orders['withdrawals']:
            trans = Transaction()
            trans.account_id = self.account_id
            trans.type = OrderType.WITHDRAWAL
            trans.external_id = o['txid']
            trans.amount = o['amount']
            trans.value_date = datetime.fromtimestamp(orders['deposits'][0]['datetime'])
            trans.currency = o['currency']
            trans.rx_address = o['destinationAddress']
            trans.fee = 0
            transactions.append(trans)
