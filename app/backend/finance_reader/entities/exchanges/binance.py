...
class Binance(AbstractExchange):
    ...
    def get_orders(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            task = {executor.submit(self.get_orders_from_binance, idx, q): q for idx, q in enumerate(symbols)}
    ...
    def create_transaction(self, order):
        ...
        # for order in order:
            new_order = Transaction()
            new_order.account_id = self.account_id
            new_order.external_id = f"{order['tranId']}"
            new_order.currency = order['coin']
            new_order.value_date = datetime.fromtimestamp(order['insertTime'] / 1000)
            new_order.type = OrderType.DEPOSIT
            new_order.rx_address = order['address']
            new_order.amount = float(order['transferAmount'])
            new_order.fee = 0
            # arr_orders.append(new_order)
            return new_order
