    ...
    entities = [
        {
            ...
            "creds": [{
                ...
                "cred_type": EntityCredentialType.Type.COOKIE_TOKEN.name,
                "mode": EntityCredentialType.Mode.PASSWORD.name
            }]
        }, {
...
def upgrade():
    ...
    conn = op.get_bind()
    query = conn.execute(sa.text("select id from entities where name = 'Degiro'"))
    degiro_entity = query.fetchone()[0]
    query = conn.execute(sa.text("select id from entities where name = 'ClickTrade'"))
    clicktrade_entity = query.fetchone()[0]
    query = conn.execute(sa.text("select id from entities where name = 'Bitstamp'"))
    bitstamp_entity = query.fetchone()[0]
    query = conn.execute(sa.text("select id from entities where name = 'Kraken'"))
    kraken_entity = query.fetchone()[0]
    query = conn.execute(sa.text("select id from entities where name = 'Bittrex'"))
    bittrex_entity = query.fetchone()[0]
    query = conn.execute(sa.text("select id from entities where name = 'Binance'"))
    binance_entity = query.fetchone()[0]
    query = conn.execute(sa.text("select id from entities where name = 'Kucoin'"))
    kucoin_entity = query.fetchone()[0]
    bulk_insert = [{
