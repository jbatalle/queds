...
class BalanceList(Resource):
    ...
    def get(self):
        ...
        wallet_items = ExchangeWallet.query.options(joinedload(ExchangeWallet.open_orders))\
...
class CryptoTax(Resource):
    ...
    def get(self):
        ...
        closed_orders = ExchangeClosedOrder.query.options(joinedload(ExchangeClosedOrder.sell_order))\
            .options(joinedload(ExchangeClosedOrder.buy_order))\
            ...
            .filter(ExchangeOrder.account_id.in_([a_id[0] for a_id in accounts])).all()

        items = []
        log.info(f"Closed orders: {len(closed_orders)}")
